...
var p = {
	...
	createHolder : function (name){
		...
		e.innerHTML = "<iframe id='" + name
				+ "' name='" + name
				+ "' src='" + this.config.path.base + "/blank.html'"
				+ " style='display : none;'"
				"><\/iframe>";
		...
		var onloadFunc = function (){
			...
			if (loader.inLoadingHolders==0){
				if (loader.config.onLoaderInited) loader.config.onLoaderInited();
			}
		}
	...
	_onFinishLoadOne : function (msg){
		...
		this.inLoading = false;
		if (this.config.onFinishLoad) this.config.onFinishLoad();
	}
}
